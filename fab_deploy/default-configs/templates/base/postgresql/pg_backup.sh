...
{% else %}
	...
	{% if mail_enable %}
	MAILCONTENT="{{ mail_option }}"

	# Set the maximum allowed email size in k. (4000 = approx 5MB email [see docs])
	MAXATTSIZE= {{ max_email_size }}
	...
	SOCKET={{ unix_socket_file }}

	# Command to run before backups
	{% if do_prebackup %}
	PREBACKUP="{{ prebackup_cmd }}"
	{% endif %}

	# Command run after backups
	{% if do_postbackup %}
	POSTBACKUP="{{ postbackup_cmd }}"
	{% endif %}
	### END CFG ###
	{% endblock %}
{% endif %}
